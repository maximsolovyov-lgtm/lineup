...
- date and split_by_day: when the request gives "run: … to …" the night lasts several days, and such a bill is nearly always published per day — day tabs, "Day 1 / Day 2", a page or column per day, names printed under Friday / Saturday / Sunday. Look for that grouping before answering: if it is there, set lineup.split_by_day = true and give EVERY line its day in date, as YYYY-MM-DD or exactly as the bill labels it ("Friday", "Day 2", "Nov 7") — the operator's form resolves those against the run. Leave the dates null and split_by_day false only when the source genuinely announces the whole run together; that is a statement about the bill, and a wrong one costs the operator the whole day-by-day structure.
