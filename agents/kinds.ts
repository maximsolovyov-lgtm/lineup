...
2. Read the roster exactly as printed: every act, in billing order, who is emphasised as the headliner, which slots are "TBA"/"TBC" (placeholder tbd) or "special/secret guest" (placeholder secret_guest), whether more names are promised ("+ more TBA" → complete = false), and the venue and date the announcement states. A page that lists acts per room (Theatre, Club Room, Main Room…) is a line-up: keep every room's acts, with room filled. A publication that spreads the acts over several VENUES (Sónar by Day at one site and by Night at another; a day at Ushuaïa continuing at Hï) is one line-up with place filled per act, as printed; acts the publication does not attribute to a venue keep place null.
