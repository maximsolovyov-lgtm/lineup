...
| `supabase db reset` runs migrations before `seed.sql`, so the backfill sees an empty database and does nothing. | `seed.sql` gained a block that derives `place_space` rows from the JSON it just inserted. Without it a freshly reset local database had venues and no rooms at all. |
