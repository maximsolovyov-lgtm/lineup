...
## Decision 2026-09-25 — the Occurrences tab, and a timetable generated from a line-up

| Question | Decision |
|---|---|
| Why a tab of its own? | Because the work happens per night, not per brand. The event owns the calendar; the Occurrences list answers "what is still missing here" — a place, a line-up, a timetable — and the form is where one night is corrected without opening its event. |
| How is an occurrence saved? | A plain `update` on `event_occurrence`, not an RPC: its children (line-ups, sets) are edited on their own screens, so there is no parent-and-children transaction to protect. `save_event_with_occurrences()` cannot be reused for one night — it takes the event's whole list and deactivates what it does not get. |
| What does "generate the pipeline" do? | It generates the **timetable** from a published line-up (`save_predicted_sets`, `20260925100000`), and touches nothing else. The line-up says who plays and in what order; the night's window says how long there is; the venue's `typical_headliner_start_time`/`_end_time` says where the top of the night sits. Everything else is an even division on the quarter hour. |
| How honest is it about being a guess? | Completely: `scenario_type = 'predicted'`, `information_origin = 'predicted'`, `confirmation_status = 'unconfirmed'`, and `confidence_score` 0.65 when the headliner slot came from the venue's own pattern against 0.45 when the plan is pure arithmetic. `scenario_version` comes from the line-up it was derived from, so a prediction always says which publication it predicted. The panel says the same in words before the operator presses the button. |
| Grouping. | Per room and per day — the two facts a line can carry (`place_space_id`, `slot_date`). Lines with no room become one group stored with `place_space_id` NULL, never the main room. For a day of a multi-day run the window is the venue's typical party window, or the same clock times as the night when it has none; the panel says which. |
| Too many acts for the night. | Every set gets the whole window rather than a five-minute slice: an overlap the operator can see beats a plausible-looking lie. Unit-tested (`plan.test.ts`), along with the even division, the pinned headliner and the fallbacks. |
| Re-generating. | One transaction for the whole timetable — a dozen `save_performance_set` calls from the browser is the per-row sequence CLAUDE.md forbids, and a failure halfway would leave half a schedule. A second run supersedes the previous prediction from that line-up (status only, no row-for-row pairing) instead of deleting it: history is what lets a prediction be compared with what happened. |
